#######################################################
#   Author:  https://github.com/Raszhagal             #
...
start_time = 0.00
alphaValue = None
configDefault = {'ImagePath': os.path.join(ROOT_DIR, 'Images_Points'),
                 'RedValue': 0,
                 'GreenValue': 0,
                 'BlueValue': 0,
                 ...
                 'FullBlending': False,
                 'FrameCount': 11,
                 'OutputPath': os.path.join(ROOT_DIR, 'Output')}
...
class MorphingApp(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.configFilePath = os.path.join(ROOT_DIR, 'config.txt')              # String used to store path for PIM's configuration file (GUI defaults)
        ...
        self.configRestoreButton.clicked.connect(lambda: self.loadConfiguration(True))
        self.configSaveButton.clicked.connect(self.saveConfiguration)
    ...
    def checkUpdate(self):
        try:
            resp = requests.get('https://api.github.com/repos/Raszhagal/Python-Image-Morpher/releases/latest')
                ...
                for x, y in zip(gitVer.split('.'), localVer.split('.')):
                    if int(x) > int(y):
                        ...
                        if userResponse == QtWidgets.QMessageBox.Yes:
                            webbrowser.open('https://github.com/Raszhagal/Python-Image-Morpher/releases/latest')
                            sys.exit()
                        return
    ...
    # Function executed on startup & restore that either generates or loads config.txt to store configuration parameters
    def loadConfiguration(self, reset):
        configVars = copy.deepcopy(configDefault)

        if not os.path.exists(self.configFilePath):  # Config file does not exist (initial startup, or deleted by user)
            with open(self.configFilePath, 'w') as file:
                for x in configVars:
                    file.write("{}={}\n".format(x, configVars[x]))
        elif not reset:  # Load config from existing file
            with open(self.configFilePath, 'r') as file:
                for line in file:
                    (key, val) = line.split('=')
                    configVars[str(key)] = val
            self.checkConfiguration()  # Validate loaded settings
        else:  # Reset config for existing file
            with open(self.configFilePath, 'w') as file:
                for x in configVars:
                    file.write("{}={}\n".format(x, configVars[x]))

        self.configImagePathString.setText(str(configVars['ImagePath']))
        self.configRedValue.setText(str(configVars['RedValue']))
        self.configRedSlider.setValue(int(configVars['RedValue']))
        self.configGreenValue.setText(str(configVars['GreenValue']))
        self.configGreenSlider.setValue(int(configVars['GreenValue']))
        self.configBlueValue.setText(str(configVars['BlueValue']))
        self.configBlueSlider.setValue(int(configVars['BlueValue']))
        self.configTriangleBool.setEnabled(bool(configVars['TrianglesEnabled']))
        self.configRGBTypeValue.setCurrentIndex(int(configVars['RGBDataType']))
        self.configTransparentBool.setEnabled(bool(configVars['Transparency']))
        self.configFullBlendBool.setEnabled(bool(configVars['FullBlending']))
        self.configFrameCountValue.setText(str(configVars['FrameCount']))
        self.configOutputPathString.setText(str(configVars['OutputPath']))

    # Function for validating configuration parameters
    def checkConfiguration(self):
        ...

    # Function that saves and applies configuration parameters
    ...
    # The alpha slider then becomes an interactive display, showing each blend in realtime.
